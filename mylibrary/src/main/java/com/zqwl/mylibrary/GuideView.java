...
public class GuideView extends RelativeLayout implements ViewTreeObserver.OnGlobalLayoutListener {
    ...
    private int targetViewLeft,targetViewTop,targetViewRight,targetViewBottom;
    ...
    public void setChangeTargetView(View targetView,int left,int top,int right,int bottom) {
        this.targetView = targetView;
        this.targetViewLeft=left;
        this.targetViewTop=top;
        this.targetViewRight=right;
        this.targetViewBottom=bottom;
    ...
    public void show() {
        ...
        this.setBackgroundResource(R.color.simple_guide_transparent);
    ...
    private void createGuideView() {
        ...
        if(null!=customGuideView&&null!=customGuideView.getLayoutParams()){
            guideViewParams =  new LayoutParams(customGuideView.getLayoutParams());
        }else{
            ...
            if (direction != null) {
                ...
                switch (direction) {
                    ...
                    case EDGE_LEFT_TOP:
                        ...
                        guideViewParams.setMargins(location[0] + offsetX, location[1] + offsetY, 0, 0);
                        break;
                    case EDGE_LEFT_BOTTOM:
                        this.setGravity(Gravity.TOP);
                        guideViewParams.setMargins(location[0] + offsetX, location[1]+targetView.getHeight() + offsetY, 0, 0);
                        break;
                }
            } else {
                guideViewParams = new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
                guideViewParams.setMargins(offsetX, offsetY, -offsetX, -offsetY);
            }

            //            guideViewLayout.addView(customGuideView);
           if(null!=location&&location.length>1&&location[0]>=0&&location[1]>=0
           ){
    ...
    private void drawBackground(Canvas canvas) {
        ...
        if (backgroundColor != 0)
            ...
            bgPaint.setColor(getResources().getColor(R.color.simple_guide_shadow));
        ...
        if (myShape != null) {
            ...
            switch (myShape) {
                ...
                case RECTANGULAR://圆角矩形
                    // 默认视图大小
                    if(isChangeView) {
                        oval.left = targetViewLeft;                              //左边
                        oval.top = targetViewTop;                                   //上边
                        oval.right = targetViewRight;                             //右边
                        oval.bottom = targetViewBottom; //下边
                    }else{
                        Rect globalRect = new Rect();
                        targetView.getGlobalVisibleRect(globalRect);
                        oval.left = globalRect.left;                              //左边
                        oval.top = globalRect.top;                                   //上边
                        oval.right = globalRect.right;                             //右边
                        oval.bottom = globalRect.bottom; //下
                    }
                    temp.drawRoundRect(oval, radius, radius, mCirclePaint);                   //绘制圆角矩形
                    break;
                case NOTARGETVIEW://圆角矩形
     ...
     */
    public enum MyShape {
        CIRCULAR, ELLIPSE, RECTANGULAR,NOTARGETVIEW
    }
    ...
    public static class Builder {
        ...
        public Builder setChangeTargetView(View target,int left,int top,int right,int bottom) {
            guiderView.setChangeTargetView(target,left,top,right,bottom);
            return instance;
        }
        ...
        public Builder setHomeGuideView(boolean home) {
//            guiderView.setDirection(Direction.RIGHT_BOTTOM);
            guiderView.setShape(MyShape.NOTARGETVIEW);
            return instance;
        }
        public Builder setChangeGuideView(boolean isChange) {
            guiderView.setChangeView(isChange);
            return instance;
        }
